...
.save-button:hover {
    background-color: #003a8c;
        ...
        .sidebar h3 { color: #0045ad; font-size: 20px; margin-bottom: 10px; }
        .sidebar-item { background-color: white; padding: 10px; border: 1px solid #ddd; margin-bottom: 20px; }
        .sidebar-item label { font-size: 16px; margin-bottom: 5px; display: block; }
        .sidebar-item input[type="number"] { width: 100%; padding: 5px; font-size: 16px; border: 1px solid #ddd; margin-top: 5px; }
        .sidebar-item button { padding: 10px 20px; border: none; cursor: pointer; font-size: 16px; color: white; background-color: #0045ad; margin-top: 10px; width: 100%; }
...
<body>
    ...
    <div class="nav">
        <div>
            ...
            <a href="{{route('showcourse')}}">KHÓA HỌC</a>
            <a href="#">HỌC VIÊN</a>
        ...
        <div class="main-content">
            ...
            <div class="test-card">
                ...
                <div class="test-card-info">
                    <h3>TEST ĐẦU VÀO ({{ session('num_questions', 10) }} câu)</h3>
                    <p>25591 lượt hoàn thành</p>
                    <p>29/06/2023 22:45</p>
                </div>
                <div class="test-card-buttons">
                    <button onclick="startTest({{ session('num_questions', 10) }})">Test ngay</button>
            ...
            <div class="test-card">
                ...
                <div class="test-card-info">
                    <h3>TEST ĐẦU VÀO ({{ session('num_questions', 20) }} câu)</h3>
                    <p>8938 lượt hoàn thành</p>
                    <p>11/08/2023 14:12</p>
                </div>
                <div class="test-card-buttons">
                    <button onclick="startTest({{ session('num_questions', 20) }})">Test ngay</button>
                    <button>Chọn từng part</button>
                </div>
            </div>
        </div>
...
<form action="{{ route('admin.updateQuestions') }}" method="POST" onsubmit="updateTestButtons(event)">
    @csrf
    <div class="sidebar-item settings-item">
    ...
    <script>
        ...
        function updateTestButtons(event) {
            event.preventDefault();
            const numQuestions = document.getElementById('num_questions').value;
            const buttons = document.querySelectorAll('.test-card-buttons button:first-child');
            buttons.forEach(button => {
                button.setAttribute('onclick', `startTest(${numQuestions})`);
            });
